...
def not_moderated():
    ...
    pagination = Review.query.order_by(Review.created_at.desc()).filter(
        Review.is_moderated == 'На рассмотрении').paginate(page, PER_PAGE)
...
def confirmed():
    page = request.args.get('page', 1, type=int)
    pagination = Review.query.order_by(Review.created_at.desc()).filter(
        Review.is_moderated == 'Одобрено').paginate(page, PER_PAGE)
    reviews = pagination.items
...
def canceled():
    page = request.args.get('page', 1, type=int)
    pagination = Review.query.order_by(Review.created_at.desc()).filter(
        Review.is_moderated == 'Отклонено').paginate(page, PER_PAGE)
    reviews = pagination.items
...
def user_reviews():
    page = request.args.get('page', 1, type=int)
    pagination = Review.query.order_by(Review.created_at.desc()).filter(
        Review.user == current_user).paginate(page, PER_PAGE)
    reviews = pagination.items
